...
- [ ] **Z is not recalled (by design):** Navigate should move **XY only** and
      leave focus alone (the manual coarse-focus knob is unsensed). Confirm it
      does NOT drive Z. Then, when roughly in focus, run autofocus to lock it.
      (Open question for later: is per-site autofocus reliable enough, or do we
      want an opt-in "autofocus after Navigate"? — see TODO "Z focus recall".)

Notes (Navigate XY accuracy in µm? autofocus behaviour when near focus?):
