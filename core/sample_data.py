...
# Base substrate (the wafer/handle) — no baked dielectric thickness: that lived in
# the old fused labels ('SiO2/Si 285nm'), was only ever a guess, and is now the
# measured numeric `sio2_nm` field.  The dielectric on top is chosen separately.
_SUBSTRATES  = ['Si', 'Sapphire', 'Quartz', 'hBN', 'Other']
# Dielectric stack on the base substrate (the "generalised contrast" axis: which
# oxide/nitride combination on Si).  Thickness(es) live in the numeric field(s).
_DIELECTRICS = ['SiO₂', 'Si₃N₄', 'SiO₂ + Si₃N₄', 'none (bare)', 'other']


def migrate_substrate(sample: dict) -> bool:
    """In-place: split a legacy fused substrate label ('SiO2/Si 285nm') into a base
    substrate + separate `dielectric` type, and ensure `dielectric` exists.

    The baked thickness is deliberately DISCARDED — old oxide numbers were guesses
    and are untrustworthy; `sio2_nm` is left untouched (re-measure by reflectance).
    Returns True if anything changed (caller should save)."""
    changed = False
    sub = str(sample.get('substrate') or '')
    if sub.startswith('SiO2/Si') or sub.lower().startswith('sio2/si'):
        sample['substrate'] = 'Si'
        sample['dielectric'] = 'SiO₂'
        changed = True
    if 'dielectric' not in sample:
        # Bare substrates carried no dielectric; default them to "none (bare)".
        sample['dielectric'] = 'none (bare)' if sample.get('substrate') in _SUBSTRATES else 'SiO₂'
        changed = True
    return changed
...
def _optical_dir(sample_dir: str) -> str:
    return os.path.join(sample_dir, 'optical')


def _git_rev() -> str | None:
    """Best-effort short git SHA for provenance (None if unavailable)."""
    try:
        import subprocess
...
def write_optical_calibration(users_root, sample: dict, cal: dict, E=None) -> str:
    """Persist a derived optical calibration into the sample's dated /optical/ store.

    Layout:  <sample>/optical/<date>_<material>_<d>nm/
                 contrast_calibration.json   — BGR targets + ladder + resolvable bands
                 E_used.csv                  — frozen snapshot of the lamp E(λ) that
                                               made these targets (reproducibility)
                 meta.json                   — oxide_nm, material, source, date, git rev

    The live lamp/references stay at app root (scope-level); this is the per-sample,
    reproducible snapshot.  Returns the contrast_calibration.json path.
    """
    material = str(cal.get('material', 'graphene'))
    d_nm = float(cal.get('sio2_nm', 0) or 0)
    sdir = _sample_dir(users_root, sample['user'], sample['folder'])
    name = f"{date.today().isoformat()}_{material}_{d_nm:g}nm"
    od = os.path.join(_optical_dir(sdir), name)
    os.makedirs(od, exist_ok=True)
    calp = os.path.join(od, 'contrast_calibration.json')
    with open(calp, 'w') as f:
        json.dump(cal, f, indent=2)
    if E is not None:
        wl, y = E
        with open(os.path.join(od, 'E_used.csv'), 'w') as f:
            f.write('wavelength_nm,E_norm\n')
            for a, b in zip(wl, y):
                f.write(f"{round(float(a), 2)},{round(float(b), 5)}\n")
    with open(os.path.join(od, 'meta.json'), 'w') as f:
        json.dump({'material': material, 'sio2_nm': d_nm,
                   'source': cal.get('source'),
                   'derived_at': datetime.now().isoformat(timespec='seconds'),
                   'git_rev': _git_rev()}, f, indent=2)
    return calp


def latest_optical_calibration(sample_dir: str):
    """Newest <sample>/optical/*/contrast_calibration.json path, or None."""
    od = _optical_dir(sample_dir)
    if not os.path.isdir(od):
        return None
    cals = [os.path.join(od, s, 'contrast_calibration.json')
            for s in os.listdir(od)
            if os.path.exists(os.path.join(od, s, 'contrast_calibration.json'))]
    return max(cals, key=os.path.getmtime) if cals else None


def find_sample_dir_upwards(start: str):
    """Walk up from a path (e.g. a scan folder) to the enclosing sample dir
    (the one containing sample.json), or None."""
...
def new_sample(users_root: str, user: str, short_name: str,
    ...
    sample = {
        ...
        'user':       user,
        'substrate':  substrate,
        'dielectric': 'SiO₂',   # dielectric stack on the base substrate (#61)
        'sio2_nm':    None,   # measured dielectric thickness (Red Tide reflectometry); #61
...
def mark_registered(sample: dict, dx_mm: float, dy_mm: float,
                    ...
                    dz_mm: float | None = None):
    """Record a confirmed placement registration.

    dz_mm: mean focus-height change between the reference corners and the
    re-registered observations (obs_z − ref_z). The chip moves as a rigid
    body on remount, so every scan-era focus height shifts by ~this amount —
    navigation adds it to stored flake/corner Z (rig finding 2026-07-03:
    ignoring it caused complete defocus after remounts).
    """
    p = _placement(sample)
    reg = p.get('registration') or {}
    reg.update({
    ...
    if dz_mm is not None:
        reg['dz_mm'] = round(dz_mm, 6)
    p['registration'] = reg


def placement_dz_mm(sample: dict) -> float:
    """Focus-height offset of the current mount vs the reference frame (mm).

    0.0 when unregistered or the registration carries no Z information.
    Consumers: z_now = stored_z + placement_dz_mm(sample).
    """
    reg = (_placement(sample).get('registration') or {})
    try:
        return float(reg.get('dz_mm') or 0.0)
